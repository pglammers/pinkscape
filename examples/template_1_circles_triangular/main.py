...
directory = os.path.join("examples", "template_1_circles_triangular")
...
for i in range(9):
    ...
    for j in range(9):
        ...
        position = transformer(np.array([i, j, 0]))
...
# We shall add one extra circle at `z=1`, to see if it works well.
position = transformer(np.array([0, 0, 1]))
c = Circle(circle_style, position, scale / 2)
svg1[f"simulation"].append(id1(c.element()))
svg2[f"simulation"].append(id2(c.element()))
